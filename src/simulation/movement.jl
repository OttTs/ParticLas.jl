function movement_step!(particles, mesh, time_step)
    ...
    @batch for threadid in 1:Threads.nthreads(:default)
        for i in threadid:Threads.nthreads(:default):length(particles.index)
            ...
            wall = nothing
            Δt = time_step
            for _ in 1:1000 # Tunneling happens a lot somehow...
                trajectory = Line(particles.position[i], Vec2{Float64}(Δt * particles.velocity[i]))
                stopindex = index(pointto(trajectory), mesh)

                next_crossing = next_wall_hit(trajectory, mesh;
                    last_wall=wall,
                    startindex=particles.index[i],
                    stopindex=stopindex
                )

                if isnothing(next_crossing)
                    particles.position[i] += trajectory.vector
                    particles.index[i] = stopindex
                    break
                else
                    # Something is wrong with walls...
                    wall, fraction = next_crossing
                    particles.position[i] += trajectory.vector * fraction
                    particles.index[i] = index(particles.position[i], mesh)
                    Δt *= (1 - fraction)
                    particles.velocity[i] =
                        collide(particles.velocity[i], wall, mesh.wall_condition)
                end
            end
            inbounds(particles.position[i], mesh) || (particles.inside[i] = false)
        end
    end
end
...
function collide(velocity, wall, wall_condition)
    normal_velocity = velocity ⋅ wall.normal

    if rand() > wall_condition.accomodation_coefficient
        return velocity - 2 * normal_velocity * wall.normal
    else
        ...
        transform_matrix = @SMatrix [ wall.normal[1]    -wall.normal[2] 0 ;
                                      wall.normal[2]     wall.normal[1] 0 ;
                                                   0                  0 1 ]
        return velocity - (transform_matrix * (v .* (sign(normal_velocity), 1, 1)))
    end
end